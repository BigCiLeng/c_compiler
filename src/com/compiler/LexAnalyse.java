...
public class LexAnalyse {
	...
	public String outputWordList() throws IOException {
		...
		pw1.println("单词\t\ttoken\t单词所在行 \t类型");
		Word word;
		for (int i = 0; i < wordList.size(); i++) {
			word = wordList.get(i);
			pw1.println(word.value + "\t\t" + word.type + "\t\t"
					+ word.line + "\t" +word.attribute);
			System.out.println(word.id + "\t\t" + word.value + "\t\t" + word.type + "\t\t"
					+ "\t" + word.line + "\t"+word.attribute);
		}
		FileOutputStream fos2 = new FileOutputStream(path + "/wordError.txt");
		BufferedOutputStream bos2 = new BufferedOutputStream(fos2);
		OutputStreamWriter osw2 = new OutputStreamWriter(bos2, "utf-8");
		PrintWriter pw2 = new PrintWriter(osw2);
		if (lexErrorFlag) {
			Error error;
			pw2.println("错误信息如下：");

			pw2.println("错误序号\t错误信息\t错误所在行 \t错误单词");
